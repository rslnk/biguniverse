jQuery(document).ready(function($) {
	...
	var psLoader = SlideshowOptions['psLoader'];
	var psHash = SlideshowOptions['psHash'];
		...
		$("div[id^=clusterone-slideshow]").each(function() {
			var num = this.id.match(/clusterone-slideshow(\d+)/)[1];
			...
			$('#clusterone-slideshow'+num).cycle({
					fx: psTrans[num],
					speed: psSpeed[num],
			...
			function onBefore(curr,next,opts) {
				var $ht = $(this).height();
				$('#clusterone-slideshow'+num).css("height", $ht);	

				$('#clusterone-slideshow'+num).animate({
					height: $ht
					}, 400, function() {
					// Animation complete.
				});
